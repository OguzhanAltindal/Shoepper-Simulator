        ...
        try (Connection conn = Database.getConnection()) {
            ...
            try {
                ...
                try (PreparedStatement ps = conn.prepareStatement(insertPHS)) {
                    for (String skill : new String[] { "Craft Gathering", "Repair Gathering", "Item Crafting",
                            "Item Repair" }) {
                        ps.setInt(1, playerId);
    ...
    public Player getPlayerByNickname(String nickname) throws SQLException {
        try (Connection conn = Database.getConnection()) {
            ...
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ...
                ResultSet rs = ps.executeQuery();
                if (rs.next())
                    return getPlayerById(rs.getInt("player_id"));
            }
        }
        return null;
    }
    ...
    public void addSkillExp(int playerId, String skillName, int expGain) throws SQLException {
        try (Connection conn = Database.getConnection()) {
            ...
            try (PreparedStatement ps = conn.prepareStatement(
                ...
                ps.setInt(1, newExp);
                ps.setInt(2, newExpNeeded);
                ps.setInt(3, playerId);
                ps.setString(4, skillName);
                ps.executeUpdate();
            }
        }
    }
